...
class RoutinePlayerViewModel {
    // MARK: - 1. Routine & Data Management
    /**
     ...
     *
     * - isDebugMode: Bool - Enables debug mode (short block durations for testing).
     */
    private let isDebugMode: Bool                  // Enables debug mode for short block durations
    
    // MARK: - 3. Playback State
    /**
     * Properties tracking the current state of playback, timers, block index, pause state, etc.
     *
     * - currentBlockIndex: Int - Index of the current block in the routine.
     * - routineStartDate: Date - Timestamp when the routine started.
     * - blockStartDate: Date - Timestamp when the current block started.
     * - isPaused: Bool - Whether the routine is currently paused.
     * - pausedDate: Date? - Timestamp when the routine was paused (if paused).
     * - totalPausedTime: TimeInterval - Total time spent paused during the routine (seconds).
     * - blockPausedTime: TimeInterval - Time spent paused during the current block (seconds).
     * - actualMeditationTimeAtPause: Int - Actual meditation time at the moment of pausing (seconds).
     * - currentTime: Date - The current time (updated by timer).
     */
    var currentBlockIndex = 0                      // Index of the current block
    var routineStartDate = Date()                  // When the routine started
    var blockStartDate = Date()                    // When the current block started
    var isPaused = false                           // Is the routine paused?
    var pausedDate: Date?                          // When the routine was paused
    var totalPausedTime: TimeInterval = 0          // Total paused time (seconds)
    var blockPausedTime: TimeInterval = 0          // Paused time in current block (seconds)
    var actualMeditationTimeAtPause: Int = 0       // Meditation time at pause (seconds)
    var currentTime = Date()                       // The current time (updated by timer)
    
    // MARK: - 4. UI State
    /**
     * Properties controlling UI alerts and display state.
     *
     * - showingEndSessionAlert: Bool - Whether to show the end session alert.
     * - showingDiscardSessionAlert: Bool - Whether to show the discard session alert.
     * - showingFinishAlert: Bool - Whether to show the finish session alert.
     */
    var showingEndSessionAlert = false             // Show end session alert?
    var showingDiscardSessionAlert = false         // Show discard session alert?
    var showingFinishAlert = false                 // Show finish session alert?
    
    // MARK: - 5. Session Tracking
    /**
     * Properties and objects for event-based session tracking and analytics.
     *
     * - sessionRecord: SessionRecord? - Event-based session record for analytics and persistence.
     */
    private var sessionRecord: SessionRecord?      // Event-based session record for analytics/persistence
    
    // MARK: - 6. Computed Properties
    /**
     * Properties that derive values from the above state (current block, progress, etc).
     *
     * - routineData: Routine - The full routine data (decoded from SavedRoutine).
     * - currentBlock: RoutineBlock? - The current block being played, or nil if complete.
     * - nextBlock: RoutineBlock? - The next block in the routine, or nil if at the end.
     * - totalBlocks: Int - Total number of blocks in the routine.
     * - elapsedTime: Int - Elapsed time since routine start, excluding paused time (seconds).
     * - inBlockProgress: Double - Progress (0.0-1.0) through the current block.
    ...
    var elapsedTime: Int {
        ...
        return max(0, Int(elapsed))                   // Elapsed time in seconds
    }
    ...
    /**
     * Functions for starting, pausing, resuming, and progressing the timer and routine blocks.
     *
     * - startTimer(): Starts the routine timer and session tracking.
     * - startCurrentBlock(): Starts the current block (reset block timer).
     * - togglePause(): Toggles pause/resume for the routine.
     * - moveToNextBlock(): Moves to the next block in the routine.
    ...
    // MARK: - 9. Session Management
    /**
     * Functions for ending, saving, or discarding sessions, and cleaning up resources.
     *
     * - endSession(saveProgress: Bool): Ends the session, optionally saving progress.
    ...
    /**
     * Utility and helper methods for updating time and other minor tasks.
     *
